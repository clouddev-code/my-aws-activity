# My AWS Activity

## 2025

### Community LT

#### EKS Auto Mode Deep Live ( JAWS-UG Yokohama Vol.81 re:invent 2024 re:cap Container 1/25)

Presented a deep dive into EKS Auto Mode.

https://speakerdeck.com/cloudevcode/eks-auto-mode-deep-live

> 👀 45 views

#### GitHub Copilot Agent to MCP Streamable HTTP (5/11)

Presented about GitHub Copilot Agent and MCP Streamable HTTP.

https://speakerdeck.com/cloudevcode/github-copilot-agenttesurumcp-streamable-httpmate

> 👀 100 views

#### Container Image Acceleration Techniques (10/15)

Presented about container image acceleration techniques.

https://speakerdeck.com/cloudevcode/presentation-kontenaimezigao-su-hua-ji-shu-dot-pptx

> 👀 20 views

#### Considered VPC Lattice but decided not to adopt it (11/1)

Presented about the experience of considering VPC Lattice but deciding not to adopt it.

https://speakerdeck.com/cloudevcode/vpc-latticejian-tao-sitaga-cai-yong-sinakatutahua-dot-pptx-dot-pptx

> 👀 18 views

#### Achieving initContainer on ECS (11/8)

Presented about how to achieve initContainer on ECS.

https://speakerdeck.com/cloudevcode/initcontainerwoecsdeshi-xian-sitai

> 👀 19 views

#### ADK for Java (11/15)

Presented about Agent Development Kit (ADK) for Java.

https://speakerdeck.com/cloudevcode/adk-for-java

> 👀 71 views ⭐ 1 star

#### Considerations for Building Grafana Loki on ECS Fargate (11/20)

Presented about considerations for building Grafana Loki on ECS Fargate.

https://speakerdeck.com/cloudevcode/grafana-lokiwoecs-fargatetegou-zhu-suruguan-dian-gong-kai-ban

> 👀 25 views

### Zenn

#### Trying Amazon Aurora DSQL from Rust (1/19)

https://zenn.dev/clouddevcode/articles/cf6694210a8a10

> ❤️ 3 likes

#### Dockerizing Python Applications (2/24)

https://zenn.dev/clouddevcode/articles/7f87c17f030281

> ❤️ 3 likes

#### Claude 3.7 Sonnet is here! Tried it from Google Cloud VertexAI (2/25)

https://zenn.dev/clouddevcode/articles/45fc5e553ed16f

> ❤️ 4 likes

#### Calling o1-mini LLM on Azure Container Apps (3/2)

https://zenn.dev/clouddevcode/articles/0c488c193301a0

> ❤️ 2 likes

#### Recognized as AWS Community Builder in Container category (3/12)

https://zenn.dev/clouddevcode/articles/74340e8abdb75f

> ❤️ 4 likes

#### First impressions of Amazon Bedrock AgentCore (7/21)

https://zenn.dev/clouddevcode/articles/53713a1bd72fb7

> ❤️ 3 likes

#### Trying GPT-OSS API Service (8/16)

https://zenn.dev/clouddevcode/articles/6785c1ec87533e

#### Invoking Bedrock with OpenAI Responses API format via Assume Role (12/6)

https://zenn.dev/clouddevcode/articles/c238ce16d390ec

#### Minimizing NAT Gateway usage with IPv6 Egress-only Internet Gateway only configuration (12/12)

https://zenn.dev/clouddevcode/articles/dd12968034b585

> ❤️ 1 like

#### TypeScript Agent Development Kit (12/12)

https://zenn.dev/clouddevcode/articles/1a9b44d5b22290

> ❤️ 1 like

#### AWS CDK now supports deploying with credentials obtained via aws login (12/14)

https://zenn.dev/clouddevcode/articles/e21d073929b93a

> ❤️ 1 like

#### ECR now automatically creates repositories when pushing images (12/21)

https://zenn.dev/clouddevcode/articles/eee955881ae066

> ❤️ 3 likes

#### AI Builders Days Slide Links (12/21)

https://zenn.dev/clouddevcode/articles/d8b7a3629b2206

> ❤️ 17 likes

#### AI Agent Platform for Business Automation using AgentCore Browser and Claude Code Skills (12/22)

https://zenn.dev/clouddevcode/articles/a6fefae1ad28fe

> ❤️ 1 like

### dev.to

#### Fargate Spot Capacity rack (3/22)

https://dev.to/shiruta/fargate-spot-capacity-rack-4ble

#### [AWS] EKS Auto Mode Node lifecycle [EKS] (5/8)

https://dev.to/shiruta/aws-eks-auto-mode-node-lifecycle-eks-1jgk

> ❤️ 1 reaction

#### I tried Strands Agent (5/18)

https://dev.to/shiruta/i-tried-strands-agent-1ik6

#### Amazon Bedrock AgentCore First impression (7/21)

https://dev.to/shiruta/amazon-bedrock-agentcore-frist-impression-lch

#### We want to achieve this configuration using only an Egress-only internet gateway with IPv6 (12/14)

https://dev.to/shiruta/we-want-to-achieve-this-configuration-using-only-an-egress-only-internet-gateway-with-ipv6-3f87

#### ECR can create automatically when image pushed (12/21)

https://dev.to/shiruta/ecr-can-create-automaticaly-when-image-pushed-38f

> ❤️ 1 reaction

---

## 2024

### Community LT

#### ECS EBS integration abstruct, use case ( JAWS-UG Container 3/7)

I explain EBS integation point, Considerations, use case, CDK Considerations

https://speakerdeck.com/cloudevcode/ecs-ebs-integation-nogai-yao-yusukesu

> 👀 960 views

#### Multiline filter Fluentbit plugin for java or Python StackTrace and so on ( JAWSUG Tokyo Launch LT 6/13)

I explain ECS logging such as awslogs, Fluentbit.
Error logs such as java stacktrace may span multiple lines.
Explains the introduction points and precautions for multi-line filters.

https://speakerdeck.com/cloudevcode/fluentbit-mulitline-filternituite

> 👀 960 views ⭐ 1 star

#### WebAssembly workload for EKS ( JAWSUG Tokyo Launch LT 9/25)

I explain how to run WebAssembly workload on EKS.

https://speakerdeck.com/cloudevcode/webassembly-workloadwoeksteli-yong-suruniha

This LT related blog is the following.

https://www.totalsolution.biz/webassembly-workload-eks/

> 👀 1.5k views

#### Considering the optimal configuration based on the re:Invent 2024 container update ( JAWSUG Yokohama LT 12/9)

Optimal configuration changed by VPC Lattice service update.
I explain about Optimal configuration and EKS Auto mode use case.

https://speakerdeck.com/cloudevcode/re-invent-2024atuhutetoteshi-xian-ke-neng-naakitekutiyawokao-eru

> 👀 300 views ⭐ 1 star

### Zenn

#### Aurora MySQL Bedrock integration

https://zenn.dev/clouddevcode/articles/a3734085e8a5f7

> ❤️ 2 likes

#### Bedrock Model invoke by Rust

https://zenn.dev/clouddevcode/articles/2b6062d8d1e46d

> ❤️ 1 like

### blog

#### ServerlessDays Day2 Workshop Join report

https://www.totalsolution.biz/serverlessdays-day2-workshop%e3%81%ab%e5%8f%82%e5%8a%a0%e3%81%97%e3%81%be%e3%81%97%e3%81%9f%e3%80%82/

#### ServerlessDays Day1 join report

https://www.totalsolution.biz/serverlessdays-day1%e3%81%ab%e5%8f%82%e5%8a%a0%e3%81%97%e3%81%be%e3%81%97%e3%81%9f/

#### Bedrock Claude Night 2 6/19

https://www.totalsolution.biz/bedrock-claude-night-2/

#### Introduction to Bedrock Development Book review

https://www.totalsolution.biz/bedrock%e9%96%8b%e7%99%ba%e5%85%a5%e9%96%80%e6%9b%b8%e7%b1%8d%e3%83%ac%e3%83%93%e3%83%a5%e3%83%bc/

#### Amazon CodeWhisperer set up. Identity Center

https://www.totalsolution.biz/amazon-codewhisperer-setup-identify-center/
